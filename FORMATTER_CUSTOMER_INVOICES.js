// n8n Code Node - Customer Invoices Formatter with Contextual Suggestions
// ใช้หลัง Execute SQL Query node สำหรับ customer_invoices action
// วางโค้ดทั้งหมดนี้ใน Code In JavaScript node

// ตรวจสอบว่ามีข้อมูลหรือไม่
const items = $input.all();

// ============================================
// กรณีที่ 1: ไม่มีข้อมูลเลย
// ============================================
if (!items || items.length === 0) {
  return {
    json: {
      reply: "ขออภัยครับ ไม่พบข้อมูลลูกค้านี้ 😔\n\n" +
             "🔍 สาเหตุที่เป็นไปได้:\n" +
             "  • ชื่อลูกค้าไม่ถูกต้อง หรือสะกดผิด\n" +
             "  • ลูกค้านี้ยังไม่มี invoice\n\n" +
             "💡 ลองคำถามอื่น:\n" +
             "  • \"การ์เดียนอินดัสทรีส์ invoice\" - ลองลูกค้าอื่น\n" +
             "  • \"สินค้าขายดีที่สุด 6 เดือน\" - ดูสินค้าขายดี\n" +
             "  • \"ศูนย์ไหนยอดขายเยอะสุด\" - ดูศูนย์ที่ขายดี\n" +
             "  • \"ยอด invoice แต่ละเดือน\" - ดูสถิติรายเดือน"
    }
  };
}

// ============================================
// กรณีที่ 2: มีข้อมูลแต่เป็น object ว่าง
// ============================================
const firstItem = items[0].json;
// ⚠️ ตรวจสอบทั้ง invoice_no และ inv_no (เพื่อรองรับทั้ง 2 format)
if (!firstItem || Object.keys(firstItem).length === 0 || (!firstItem.invoice_no && !firstItem.inv_no)) {
  return {
    json: {
      reply: "ขออภัยครับ ไม่พบ invoice ของลูกค้านี้ 😔\n\n" +
             "💡 ลองคำถามอื่น:\n" +
             "  • \"การ์เดียนอินดัสทรีส์ invoice\" - ลองลูกค้าอื่น\n" +
             "  • \"สินค้าขายดีที่สุด 6 เดือน\"\n" +
             "  • \"ศูนย์ไหนยอดขายเยอะสุด\"\n" +
             "  • \"ยอด invoice แต่ละเดือน\""
    }
  };
}

// ============================================
// กรณีที่ 3: มีข้อมูล - Format response
// ============================================
const invoices = items.map(item => item.json);
const firstInvoice = invoices[0];
const customerName = firstInvoice.customer_name || firstInvoice.customer_no || 'ไม่ระบุ';
const customerNo = firstInvoice.customer_no || '';

// คำนวณยอดรวม
const totalAmount = invoices.reduce((sum, inv) => {
  const amount = parseFloat(inv.total_amount || 0);
  return sum + amount;
}, 0);

let reply = `📋 Invoice ของลูกค้า: ${customerName}`;
if (customerNo) {
  reply += ` (${customerNo})`;
}
reply += '\n\n';

reply += `จำนวนทั้งหมด: ${invoices.length} รายการ\n`;
reply += `ยอดรวมทั้งหมด: ${totalAmount.toLocaleString('th-TH', {minimumFractionDigits: 2})} บาท\n\n`;
reply += 'รายการล่าสุด:\n';

// แสดง invoice สูงสุด 10 รายการ
invoices.slice(0, 10).forEach((invoice, index) => {
  // ⚠️ รองรับทั้ง invoice_no และ inv_no (เพื่อรองรับทั้ง 2 format)
  const invNo = invoice.invoice_no || invoice.inv_no || 'N/A';
  const invDate = invoice.inv_date || invoice.inv_posting_date || 'N/A';
  const amount = parseFloat(invoice.total_amount || 0);
  
  reply += `${index + 1}. ${invNo} - ${invDate} - ${amount.toLocaleString('th-TH', {minimumFractionDigits: 2})} บาท\n`;
});

// ถ้ามีมากกว่า 10 รายการ
if (invoices.length > 10) {
  reply += `\n... และอีก ${invoices.length - 10} รายการ\n`;
}

reply += '\n━━━━━━━━━━━━━━━━━━━━━━━━\n';
reply += '💡 ต้องการดูข้อมูลเพิ่มเติม?\n';

// Contextual suggestions
// ⚠️ รองรับทั้ง invoice_no และ inv_no (เพื่อรองรับทั้ง 2 format)
const firstInvNo = invoices[0].invoice_no || invoices[0].inv_no;
if (firstInvNo) {
  reply += `• "${firstInvNo}" - ดูรายละเอียด invoice นี้\n`;
}
reply += '• "ลูกค้าไหนยอดเยอะสุด" - ดูลูกค้ายอดเยอะสุด\n';
reply += '• "ยอด invoice แต่ละเดือน" - ดูสถิติรายเดือน\n';
reply += '• "สินค้าขายดีที่สุด 6 เดือน" - ดูสินค้าขายดี\n';

// ถ้ามีชื่อลูกค้า แนะนำให้ดูสินค้าที่ลูกค้านี้ซื้อ
if (customerName && customerName !== 'ไม่ระบุ') {
  reply += `• "สินค้าอะไรที่ ${customerName} ซื้อบ่อย" - วิเคราะห์สินค้า`;
}

return { json: { reply: reply } };
